...
def test_create_soft_delete_hides_claim_and_writes_audit(api_key_header):
    ...
    # 2) Soft delete
    r = client.delete(f"/claims/{claim_id}", headers=api_key_header)
    assert r.status_code == 200, r.text

    # 3) Fetch default (include_deleted=false) -> should be hidden (404)
    r = client.get(f"/claims/{claim_id}?include_deleted=false", headers=api_key_header)
    assert r.status_code == 404, r.text

    # 4) Fetch include_deleted=true -> should exist and have deleted_at
    r = client.get(f"/claims/{claim_id}?include_deleted=true", headers=api_key_header)
    assert r.status_code == 200, r.text
    deleted = r.json()
    assert deleted["deleted_at"] is not None

    # 5) Audit log should include delete of deleted_at
    r = client.get(f"/claims/{claim_id}/audit?limit=50&offset=0", headers=api_key_header)
    assert r.status_code == 200, r.text
    logs = r.json()
    assert isinstance(logs, list)

    # Look for the delete audit record
    matches = [x for x in logs if x.get("action") == "delete" and x.get("field") == "deleted_at"]
